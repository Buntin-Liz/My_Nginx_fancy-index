...
function createBreadcrumbs(path) {
    ...
    var hostname = window.location.host
        .split(".")
        .map(function (c) {
            return c.indexOf("xn--") == 0 ? punycode.decode(c.substring(4)) : c;
        })
        .join(".");
...
function enhance(path) {
    ...
    for (var j = 1; j < $items.length; j++) {
        ...
        var icon = "/fancy-index/themes/Humanity/mimes/48/" + (mimetype ? mimetype.replace("/", "-") : "application-octet-stream") + ".svg";
        var RE_DELIMITERS = /[ \.\-\_]/;
        ...
        if (filename[filename.length - 1] == "/") {
            ...
            var $video = $(
                '<a class="thumbnail jpeg fancybox" data-mimetype="' +
                    mimetype +
                    '" data-size="' +
                    size +
                    '" data-filename="' +
            ...
            // SVG files
            $gallery.append(
                '<a class="thumbnail fancybox svg" title="' +
                    basename +
                    '" rel="g" href="' +
                    url +
                    '"><object type="image/svg+xml" data="' +
                    filename +
                    '">Bam! No SVG support!</object><p>' +
                    filename +
                    "</p></a>"
            );
            ...
            $gallery.append(
                '<a class="thumbnail fancybox jpeg" title="' + basename + '" rel="g" href=".moar/scaled/' + url + '"><img src=".moar/thumbs/' + filename + '"/><p>' + filename + "</p></a>"
            );
        } else if (IMAGE_EXTENSIONS.indexOf(extension) >= 0) {
            // Other images
            basename = basename.toLowerCase();
            if (basename.substring(0, 3) == "img") basename = basename.replace("_", "");
    ...
    $("#actions .directory .action.wget.recurse")
        .click(function (e) {
        ...
        $.ajax("index.xspf").done(function (data) {
            ...
            for (var j = 0; j < $trackMetadatas.size(); j++) {
                ...
                if ($trackCreator.html() && $trackTitle.html()) {
                    ...
                    $track.attr("title", $trackTitle.html() + " (" + humanize_duration(duration) + ") by " + $trackCreator.html() + " on " + $trackAlbum.html());
                }
            }
